...
def _buckets_for_length(bucket_length, batch_size, max_eval_length,
                        training):
...
def batcher(data_streams, variable_shapes=True,
  ...
  batch_train_stream = batch_fn(
      train_stream(), True, variable_shapes,
  ...
  batch_eval_stream = batch_fn(
      eval_stream(), False, variable_shapes,
      batch_size_per_device, batch_size, eval_batch_size,
      bucket_length, buckets, buckets_include_inputs_in_length,
      batch_shuffle_size, max_eval_length, id_to_mask, strict_pad_on_len, len_fn, add_weights_fn)
  ...
  return batch_train_stream, batch_eval_stream, 49


def batch_fn(dataset, training, variable_shapes,
             batch_size_per_device=32, batch_size=None, eval_batch_size=32,
             bucket_length=32, buckets=None,
             buckets_include_inputs_in_length=False,
             batch_shuffle_size=None, max_eval_length=None,
  ...
  batch_size = batch_size or batch_size_per_device
  ...
  cur_batch_size = max(cur_batch_size, 1)
  ...
  if buckets is None:
    ...
    if variable_shapes:
      buckets = _buckets_for_length(
          bucket_length, cur_batch_size, max_eval_length, training)
